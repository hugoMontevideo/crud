...
if (isset($_GET['i'])){
    ...
    if ( isset($_GET['a'])  &&  $_GET['a'] == 'role'){
        $user = execute( "SELECT role FROM user WHERE id=:id", 
                     array( ':id' => $_GET['i'] ))->fetch(PDO::FETCH_ASSOC);

        if ($user['role']=='ROLE_USER'){
            execute(" UPDATE user SET role=:role WHERE id=:id",
                    array( 
                        ':role'=>'ROLE_ADMIN', 
                        ':id' => $_GET['i'] 
                    ));
        ...
        <tr>
            ...
            <td>
                    ...
                    <a 
                        ...
                        class="btn btn-info "
                        style="font-size: .5em;padding: 1.45em .5em;"
